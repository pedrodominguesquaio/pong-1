let xb=300;
let yb=200;
let diametro=15;
let raio=diametro/2;
let vxb=6;
let vyb=6;

let xr=5;
let yr=150;
let lr=7
let ar=80;

let xri=585;
let yri=150;

let colidiu= false;

let meuspontos=0;
let pontosoponente=0;
let trilha;
let ponto;
let raquetada;

function preload(){
  trilha= loadSound('trilha.mp3');
  ponto= loadSound('ponto.mp3');
 raquetada= loadSound('raquete.mp3');
}
  

function setup() {
  createCanvas(600, 400);
trilha.loop()
}

function draw() {
  background("black");
  mostrabola();
  mostraraquete(xr,yr,color('rgb(10,10,44)'));
  mostraraquete(xri,yri,color('rgb(77,11,11)'));
  mexebola();
  quicabola();
  mexeraquete();
  mexeraqueteinimiga();
  quicaraquetebola(xr,yr);
  quicaraquetebola(xri,yri);
  placar();
  pontos();
  letras();
  stroke('red');
  rect(300,0,1,600);
  
}

function mostrabola(){
  circle(xb,yb,diametro);
}

function mostraraquete(x,y,color){
  fill(color);
  rect(x,y,lr,ar);
}
  
  
  function mexebola(){
    xb += vxb;
    yb +=vxb;
  }
  
  function quicabola(){
    if (xb+raio>width || xb-raio<0){
      vxb *= -1;
    }
  
  if (yb+raio>height || yb-raio<0){
      vyb *= -1;
    }
  }
  
  function mexeraquete(){
    if (keyIsDown(UP_ARROW))
        yr-=10;
        
        if(keyIsDown(DOWN_ARROW))
      yr += 10;
  }
    
    
    function mexeraqueteinimiga(){
    if (keyIsDown(87))
        yri-=10;
        
        if(keyIsDown(83))
      yri += 10;
    }
function quicaraquetebola(x,y){
colidiu=collideRectCircle(x,y,lr,ar,bx,xb,raio);
if(colidiu){
  vxb*=-1;
  raquetada.play();
}
}
  function placar(){
    stroke('red')
    textAlign(CENTER);
    textSize(18);
    fill('blue')
    rect(150,10,40,20);
  fill(meuspontos,170,15);
  fill('#101609')
    rect(430,10,40,20);
  fill(pontosoponente,450,15);
  }
  function pontos(){
    if (xb>590){
      meuspontos+= 1;
      ponto.play();
}
if (xb < 11){
      pontosoponente += 1;
}    
}
function letras() {
  let frase = 'MEUS PONTOS'
  let frase2='PONTOS OPONENTE';
  textAlign(LEFT,TOP);
fill('green')
text(frase,90,40);
  textAlign(LEFT,TOP);
fill("green")
text(frase2,335,40);
}